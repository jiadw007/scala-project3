...
class Node(val index: Int, var numRequest: Int, var id: NodeId, val b: Int = 2,val L:Int =16) extends Actor with Ordered[Node] {
  ...
  def findActorRef(nodeId: NodeId) = {
    
    context.actorSelection(s"/user/NodeInstance${Node.mapNodeActor(nodeId.bits)}")
  }
  
  
  def UpdateActorRef(nodeId: NodeId, index: Int) = {
    Node.mapNodeActor(nodeId.bits) = index
  }
  ...
  def receive()= {
    ...
    case Node.initial =>{
     ...
     //A node with key = this.id and index = this.index join
    case Node.join(key, index, hop, response) =>{
      ...
      if(inleaf){
        ...
        if(this.routingTable(row)(col) ==null){
          ...
          //First froward to all nodes we knew which is closet to key
    ...
    case Node.newRouting(routingTable2, nodeId, hop) =>{
      ...
      for(i <- 0 to rows - 1; j<- 0 to cols-1){
        ...
        if(this.routingTable(i)(j) ==null &&routingTable2(i)(j) !=null &&routingTable2(i)(j).bits != -999 && this.id.getCommonPrefixLength(routingTable2(i)(j)) >= i){
    ...
    case Node.route(msg, key) =>{
      ...
      //check if next is in the leaf set
      if(key.bits >= fullSet.map(p=>p.bits).min && key.bits <= fullSet.map(p =>p.bits).max){
        ...
        //check if this node is the closet one 
    ...
    case Node.start =>{
      ...
      if(this.index ==0){
        ...
        println(s"Request ${this.numSent}")
